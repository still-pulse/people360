...
export function AdmissionDashboard() {
  ...
  return <><Header title="Admissão Digital" subtitle="Gestão segura do processo admissional"/><div className={styles.module}><div className={styles.content}>
    <AdmissionTitle tourId="overview-heading" actionsTourId="overview-actions" title="Admissão Digital" subtitle="Acompanhe todas as admissões em andamento, revise documentos e integre os novos colaboradores ao ERPNext.">
    ...
    {loading?<LoadingCards count={8}/>:error?<ErrorState message={error} retry={load}/>:data&&<>
      <div className={styles.kpiGrid} data-admission-tour="overview-indicators">{metrics.map(([key,label,color])=><div className={styles.kpi} style={{'--accent':color} as React.CSSProperties} key={key}><div className={styles.kpiLabel}>{label}</div><div className={styles.kpiValue}>{data.indicators[key]??0}</div><div className={styles.kpiMeta}>Período selecionado</div></div>)}</div>
      <div className={styles.grid2} data-admission-tour="overview-stages">
      ...
      <div className={styles.grid2} data-admission-tour="overview-attention">
