...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.engagementIndicator.findMany({
    where,
    include: { unit: true },
...
export async function POST(req: NextRequest) {
  ...
  const data = await prisma.engagementIndicator.upsert({
    where: { unitId_year_month: { unitId: body.unitId, year: body.year, month: body.month } },
    update: { rate: body.rate, totalRespondents: body.totalRespondents, totalEmployees: body.totalEmployees },
    create: { unitId: body.unitId, year: body.year, month: body.month, rate: body.rate, totalRespondents: body.totalRespondents, totalEmployees: body.totalEmployees },
    include: { unit: true },
  })
