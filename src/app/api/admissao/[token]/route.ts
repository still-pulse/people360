...
const saveSchema = z.object({ section: z.enum(['personal', 'address', 'bank']), fields: z.record(z.union([z.string().max(500), z.boolean(), z.number(), z.null()])), nextStep: z.string().max(40).optional() })
...
export async function PATCH(req: NextRequest, { params }: { params: { token: string } }) {
  ...
  const cpf = parsed.data.fields.cpf; if (typeof cpf === 'string' && !isValidCpf(cpf)) return NextResponse.json({ error: 'CPF inválido.', fields: { cpf: ['Informe um CPF válido.'] } }, { status: 400 })
  await prisma.$transaction([
    ...entries.map(([key, value]) => { const sensitive = SENSITIVE_FIELD_KEYS.has(key); const stored = sensitive ? encryptAdmissionValue(value) : value; const searchHash = key === 'cpf' && typeof value === 'string' ? hashSensitive(value) : null; return prisma.admissionField.upsert({ where: { admissionId_key: { admissionId: result.admissionId, key } }, create: { admissionId: result.admissionId, section: parsed.data.section, key, value: stored as Prisma.InputJsonValue, sensitive, searchHash }, update: { section: parsed.data.section, value: stored as Prisma.InputJsonValue, sensitive, searchHash } }) }),
