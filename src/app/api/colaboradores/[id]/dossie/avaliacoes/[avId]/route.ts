...
import { avaliacaoSchema, avaliacaoView, cancelAvaliacao, saveAvaliacao } from '@/lib/dossie/avaliacoes'
...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string; avId: string }> }) {
  ...
  return dossieRoute(req, 'employee.documents.view', params.id, async ({ colaborador }) => {
    ...
    return NextResponse.json(avaliacaoView(row))
  })
}

/** Salva rascunho ou finaliza (`finalizar: true`). Avaliações finalizadas ficam imutáveis. */
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string; avId: string }> }) {
  const params = await props.params;
...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string; avId: string }> }) {
  const params = await props.params;
